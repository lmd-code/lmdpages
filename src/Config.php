...
class Config
{
    ...
    public static function getPage(string $key = '')
    {
        if (is_null(self::$page)) {
            if (!self::$route || self::$route === 'error404' || empty(self::$data['pages'])) {
                ...
                if (array_key_exists($key, self::$errorPage)) {
                    return self::$errorPage[$key];
                }
                return null;
            }
        ...
        if (array_key_exists($key, self::$page)) {
        ...
        if (!is_array(self::$scripts)) {
            ...
            foreach (self::get('pages') as $key => $val) {
                if (empty($val['scripts'])) {
                    continue; // skip if there are no page scripts
                }
    ...
    public static function getStyles(): array
    {
        if (!is_array(self::$styles)) {
            ...
            foreach (self::get('pages') as $key => $val) {
                if (empty($val['styles'])) {
                    continue; // skip if there are no page styles
                }
